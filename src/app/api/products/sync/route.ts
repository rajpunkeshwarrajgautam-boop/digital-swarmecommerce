...
export async function POST() {
  try {
    ...
    // Mirrors src/lib/data.ts but uses snake_case for Supabase columns.
    // Run this sync whenever data.ts is updated so the DB stays in step.
    const catalog = [
      {
    ...
    // onConflict: 'name' requires a unique index on the name column in Supabase.
    // If that doesn't exist, use the insert + delete-old approach below.
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      local_catalog_count: 11,
      in_sync: (count ?? 0) >= 11,
      message: count === 0
        ? 'Database is empty — run POST /api/products/sync to populate.'
        : `${count} products in Supabase. ${(count ?? 0) >= 11 ? 'In sync ✓' : 'Run sync to update.'}`,
    });
  } catch (err) {
