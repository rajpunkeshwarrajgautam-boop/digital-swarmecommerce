...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!product || !product.inStock || !product.downloadUrl) {
    ...
    const licenseKey = createLicenseKey(orderId, customerEmail, requestedProductId);
    ...
    if (!existingCommission) {
      try {
        ...
        await sealTransaction({
          ...
          payload: { orderId, splits, timestamp: new Date().toISOString() },
        });
          ...
          html: `
            <div style="font-family:Arial,sans-serif;background:#07070b;color:#f6f1e8;padding:40px">
              <div style="max-width:640px;margin:auto;border:1px solid #302b22;border-radius:20px;padding:32px;background:#0c0c12">
                ...
                <p style="color:#b7b3aa;line-height:1.7">Your paid asset is available through the private link below for 72 hours.</p>
